...
if (IS_WIN && empty($environment['SystemRoot'])) {
...
$exts_to_test = array_map('strtolower', $exts_to_test);
...
";
	...
	if (!empty($section_text['GET']) || !empty($section_text['POST']) || !empty($section_text['GZIP_POST']) || !empty($section_text['DEFLATE_POST']) || !empty($section_text['POST_RAW']) || !empty($section_text['PUT']) || !empty($section_text['COOKIE']) || !empty($section_text['EXPECTHEADERS'])) {
		if (isset($php_cgi)) {
			$old_php = $php;
			$php = $php_cgi . ' -C ';
		} else if (!IS_WIN && file_exists(dirname($php) . "/php-cgi.exe")) {
			$old_php = $php;
	...
	if (array_key_exists('PHPDBG', $section_text)) {
		...
		if (isset($phpdbg)) {
			$old_php = $php;
			$php = $phpdbg . ' -qIb';
		} else if (!IS_WIN && file_exists(dirname($php) . "/phpdbg.exe")) {
			$old_php = $php;
